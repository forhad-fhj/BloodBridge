  ...
  const handleGoogleLogin = () => {
    googleSignIn()
      .then(async (result) => {
        ...
        try {
          // Check if user exists, if not set default role as donor
          const checkUser = await axiosSecure.get("/get-user-role").catch(() => null);
          
          if (!checkUser) {
            // New user - sync to database with default role: donor
          ...
          console.error("Google login DB sync failed", err);
        }
